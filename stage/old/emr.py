...
SPARK_STEPS = [
    ...
    },
    {
      ...
      'HadoopJarStep': {
          'Jar': 'command-runner.jar',
          'Args': ['spark-submit','{{ dag_run.conf["aggregate_script"] }}','{{ dag_run.conf["aggregate_output"] }}','{{ task_instance.xcom_pull(task_ids="copyFile", key="return_value") }}']
    },
    }
]
...
def  ny_taxi_Pipeline():
    ...
    s3_file_check >> validateFile >> renameFile >> copyFile >> create_emr_cluster  >> step_adder >>[ step_checker_hive_task , step_checker_agg_task ]>> cluster_remover 
nytaxi=ny_taxi_Pipeline()
